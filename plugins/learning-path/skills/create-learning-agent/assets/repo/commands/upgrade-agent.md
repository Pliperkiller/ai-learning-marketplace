...
6. Entrega el plan: escríbelo en `docs/upgrades/YYYY-MM-DD-v<actual>-a-v<nueva>.md` (formato: `docs/upgrades/_plantilla.md`, o `assets/repo/plantilla-plan-upgrade.md` de la skill si el repo aún no la tiene) y en el chat muestra solo un resumen + "Lee el plan en `<ruta>` y responde `aprobado`, o dime qué cambiar". Si el entorno no te deja escribir archivos (plan mode), presenta el plan por el mecanismo de plan y escribe el archivo como primer paso de la fase 2.
