...
3. Aplica el cambio respetando estos guardarraíles:
   ...
   - Cambios a `CLAUDE.md` no pueden romper los invariantes: protocolo de sesión, progresión de estados, repetición espaciada, cierre con commit+push, gate de `/setup`, el gate de entrada de fase (retos verificados, nunca autorreporte; capstone y criterios de dominio intocables), las convenciones del vault de Obsidian en `material/`, ni el gate de aprobación de `/upgrade-agent`.
