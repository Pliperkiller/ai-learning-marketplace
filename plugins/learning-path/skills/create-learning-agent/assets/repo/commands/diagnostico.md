...
Cierra con `progress.json` poblado (nivel y status por tópico, niveles por fase, nivel global, fortalezas, debilidades, posicion_actual, y `gates_fase.<fase de posicion_actual>` con `origen: "diagnostico"` — el sondeo ya recorrió esa fase, así que no recibe gate de entrada; el gate arranca en la siguiente), el frontmatter de las notas de tópico sincronizado, nota de sesión en `material/sesiones/`, commit y push, y una tabla fase → nivel en el chat.
